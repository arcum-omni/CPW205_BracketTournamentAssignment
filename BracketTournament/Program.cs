...
namespace BracketTournament
{
    class Program
    {
        ...
        {
            //  TODO: Psuedo code map for assignment
            ...
            //string fileName = "largeTournament.txt";
            ...
            Console.Write("Bracket Entry Sign Up Form: ");
            ...
            if (numParticipants % 8 == 0)
            {
                ...
                Console.WriteLine("There are enough entries to form " + numberOfBrackets + " " + singularPlural + " in the brackets tournament");
                StartBracketTournament(txtArray);
            }
            else if (numParticipants % 8 != 0)
            {
                Console.WriteLine("We need " + participantsNeeded + " additional entries to start a brackets tournament");
            }
        }

        public static void StartBracketTournament(string[] txtArray)
        {
            // Random Number Generator
            var rand = new Random();

            int numParticipants = txtArray.Length;
            ...
            /*
                ...
                */
            int[] bracketPositions = new int[numParticipants];
            for (int i=0; i<numParticipants; i++)
            {
                bracketPositions[i] = -1;
            }

            for (int i = 0; i < bracketPositions.Length; i++)
            {
                int tempNum = rand.Next(numParticipants);
                while (bracketPositions.Contains(tempNum))
                {
                    tempNum = rand.Next(numParticipants);
                }
